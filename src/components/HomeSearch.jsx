"use client"; // all the time when we have a responsive button or any state that is responsive/making something we need to transform that file in a 'client side'
...
export default function HomeSearch() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!input.trim()) return; //if the Input is empty just return
  ...
  const randomSearch = async (e) => {
    ...
    const response = await fetch("https://random-word-api.herokuapp.com/word")
      .then((res) => res.json())
      .then((data) => data[0]); //taking only the base word, the 0, first word
    if (!response) return;
    ...
    <>
      ...
      >
        ...
        <input
          ...
          //👃👃[BODY CONTENT]👃👃 onChange={...} this is how we take the data/info's that the User Typed in the Input field
        />
